...
class Firewall:
    ...
    def check_hit(self, delay: int) -> bool:
        """ Returns severity taking into account current state """
        index = (self.depth + delay) % (self.width * 2 - 2)
        return index == 0

def read_input(input_file_name: str) -> List[Firewall]:
    """ Reads firewalls data from the input file """

    firewalls = list()
    with open(input_file_name) as input_file:
        for line in input_file.readlines():
            depth, width = line.rstrip('\n').split(': ')
            firewalls.append(Firewall(int(depth), int(width)))

    return firewalls

def go_through_firewalls(firewalls: List[Firewall]) -> int:
    """ Passes through all firewalls and returns a severity of that trip """

    severity = 0
    for firewall in firewalls:
        if firewall.check_hit(0):
            severity += firewall.depth * firewall.width

    return severity

def check_hits(firewalls: List[Firewall], delay: int) -> bool:
    """ Checks if packet will hit any firewall after the specified delay """

    for firewall in firewalls:
        if firewall.check_hit(delay):
            return True
...
def main():
    ...
    # read input
    firewalls = read_input('day_13_input.txt')

    delay = 0
    severity = go_through_firewalls(firewalls)
    print(severity)

    while check_hits(firewalls, delay):
        delay += 1
