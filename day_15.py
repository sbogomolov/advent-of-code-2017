...
class Generator:
    def __init__(self, first_value: int, factor: int):
...
class GeneratorWithMultiple:
    def __init__(self, first_value: int, factor: int, multiple_of: int):
        self.value = first_value
        self.factor = factor
...
def get_last_16_bits(value: int) -> int:
    return value & 0b00000000000000001111111111111111


def main():
    generator_a_init = 512
    generator_b_init = 191

    # part 1
    generator_a = Generator(generator_a_init, 16807)
    generator_b = Generator(generator_b_init, 48271)

    score = 0
    for i in range(40_000_000):
        value_a = generator_a.next()
        value_b = generator_b.next()
        if get_last_16_bits(value_a) == get_last_16_bits(value_b):
            score += 1

    print(score)

    # part 2
    generator_a = GeneratorWithMultiple(generator_a_init, 16807, 4)
    generator_b = GeneratorWithMultiple(generator_b_init, 48271, 8)
